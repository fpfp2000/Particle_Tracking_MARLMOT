...
from working_dataloader import TrackDataloader
...
def get_args():
    ...
    parser.add_argument("--txt_files", dest="txt_files", type=str, default=r"/Users/fpfp2/Desktop/Masters Thesis/CV_tracking/Particle_Tracking_MARLMOT/working_on_new/working code") 
...
def get_sort_rollout(world, dataloader, iou_threshold, min_age):
    ...
    # Access the necessary values directly from TestWorld
    ground_truth = world.ground_truth
    detections = world.detections
    gt_data = world.gt_data
    frame_size = world.frame_size

    # Initialize the SORT tracker
    # print(f"Min Age Type: {type(min_age)}, Value: {min_age}")
    tracker = HungarianTracker(iou_threshold=iou_threshold, min_age=min_age)

    with open(txt_files, 'w') as f_out:
        
        # Iterate through each frame
        world.frame = 0
        while world.frame < len(detections.frame.unique()):  # Process each frame
            # Get detections for the current frame
            frame_detections = detections[detections.frame == world.frame]

            if frame_detections.empty:
                world.frame += 1
                continue

            # Update the SORT tracker with the detections from the current frame
            sort_tracks = tracker.update(frame_detections)

            # Track metrics
            total_num_tracks += len(ground_truth[ground_truth.frame == world.frame])

            for track in sort_tracks:
                track_id = track.id
                bbox = track.bbox

                f_out.write(f"{world.frame}, {track.id}, {bbox[0]}, {bbox[1]}, {bbox[2]}, {bbox[3]}\n")

            # Get metrics
            num_false_positives += len(world.false_positives) if hasattr(world, 'false_positives') else 0
            num_false_negatives += len(world.missed_tracks) if hasattr(world, 'missed_tracks') else 0
            num_mismatch_errors += world.mismatch_errors if hasattr(world, 'mismatch_errors') else 0
            cost_penalties += world.cost_penalty if hasattr(world, 'cost_penalty') else 0

            # Add this frame's observations 
            batch_obs += [track.observation for track in sort_tracks]  

            # Use default actions for SORT
            actions, logprobs = get_sort_actions({track.id: track.observation for track in sort_tracks})
    ...
    mota = 1 - ((num_false_positives + num_false_negatives + num_mismatch_errors)) / total_num_tracks

    # Return the sort_tracks and metrics
    return sort_tracks, {
        "mota": mota,
        "false_positives": num_false_positives,
        "false_negatives": num_false_negatives,
        "mismatch_errors": num_mismatch_errors,
