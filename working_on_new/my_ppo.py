...
###############CHANGES HERE#############################################################################################
    ...
    def batch_rollout(self):
        ...
        for (ground_truth, detections, gt_data, frame_size, frame_paths) in self.dataloader:
            print(f"Ground truth: {ground_truth}, Detection: {detections}")

            if ground_truth is None or detections is None:
                print("Ground truth or detections are missing!")
                continue
            ...
            while True:    
                ...
                # for idx, (detection_row, truth_row) in enumerate(zip(detections.iterrows(), ground_truth.iterrows())):
                #     detection = detection_row[1]
                #     truth = truth_row[1]

                #     # print(f"Detection: {detection}, Truth: {truth}")
                ...
                #         if not self.is_matching(detection, truth):
    ...
    # def is_matching(self, detection, truth):

    #     # Extract bounding box from detection
    #     detection_bbox = [
    #         detection['bb_left'], 
    #         detection['bb_top'], 
    #         detection['bb_left'] + detection['bb_width'], 
    #         detection['bb_top'] + detection['bb_height']
    #     ]
    ...
    #         truth['bb_left'], 
    #         truth['bb_top'], 
    #         truth['bb_left'] + truth['bb_width'], 
    #         truth['bb_top'] + truth['bb_height']
    #     ]

    #     iou = compute_iou(detection_bbox, truth_bbox)
    #     iou_threshold = 0.5
