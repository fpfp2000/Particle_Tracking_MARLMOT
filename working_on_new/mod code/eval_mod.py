...
def get_sort_rollout(dataloader, iou_threshold, min_age):
    ...
    for (ground_truth, detections, frame_size) in dataloader:
        ...
        if not isinstance(frame_size, (tuple, list)) or len(frame_size) != 2:
            raise ValueError(f"frame_size is invalid: {frame_size}. It should be a tuple or list of two elements (height, width).")
...
def eval_sort(dataloader, iou_threshold, min_age):
    ...
    if isinstance(mismatch_errrors, list):
        mismatch_errrors = mismatch_errrors[0]

    if isinstance(cost_penalty, list):
        cost_penalty = cost_penalty[0]
...
def eval_marlmot(dataloader, policy_path, iou_threshold, min_age):
    ...
    print("action ratios: ", np.array(ppo.metrics["action_ratios"]))
    print("false positives: ", false_positives)
    print("false negatives: ", false_negatives)
    print("mismatch errrors: ", mismatch_errrors)
    print("cost penalty: ", cost_penalty)
