...
def save_tracking_results(tracked_objects, savepath):
    os.makedirs(os.path.dirname(savepath), exist_ok= True)

    with open(savepath, mode='w', newline='') as file:
        writer = csv.writer(file)

        writer.writerow(['tracke_id', 'x1', 'y1', 'x2', 'y2'])
...
def draw_sort_tracks(frame, tracks):
    ...
    for track in tracks:
        ...
        # draw track info
        label = f"SORT_{track_id}" #_{track.age}"
